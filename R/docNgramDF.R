...
##     }
  ...
  #
   ...
   opt <- paste(dic, mecabrc, etc)
    ...
    dtm <- .Call("docNgramDF", as.character(mojiVec), as.numeric(rowN), as.integer(type),  as.character(pos),  as.integer(posN), as.integer(minFreq), as.integer(N),  kigo = 0,  as.character(opt), PACKAGE = "RMeCabUni")
