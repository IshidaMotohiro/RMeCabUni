...
##     }
   ...
   opt <- paste(dic, mecabrc, etc)
    ...
    dtm <- .Call("docNgram2", as.character(directory), as.character(file), as.numeric(fileN), as.integer(ft), as.integer(type),  as.character(pos), as.integer(posN), as.integer(minFreq), as.integer(N), as.integer(kigo), as.character(opt), PACKAGE="RMeCabUni")# as.integer(sym), as.character(kigo), 
