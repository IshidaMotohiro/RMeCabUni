...
##     }
   ...
   opt <- paste(dic, mecabrc, etc)
    ...
    dtm <- .Call("NgramDF2", as.character(directory), as.character(file), as.numeric(fileN), as.integer(ft), as.integer(type),  as.character(pos), as.integer(posN), as.integer(minFreq), as.integer(N), as.numeric(kigo), as.character(opt),  PACKAGE="RMeCabUni")
