...
##     }
   ...
   opt <- paste(dic, mecabrc, etc)
    ...
    dtm <- .Call("docMatrix2", as.character(directory), as.character(file), as.numeric(fileN), as.numeric(ft), as.character(pos), as.numeric(posN),  as.numeric(minFreq), as.numeric(kigo), as.character(opt), PACKAGE="RMeCabUni")## as.numeric(sym),  as.character(kigo), 
