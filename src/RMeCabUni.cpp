/*
  ver 0.9995 2014 09 22 unidic対応
  #unidic
  [[24]]
 [1] "汽車"     "名詞"     "普通名詞" "一般"     "*"        "*"       
 [7] "*"        "キシャ"   "汽車"     "汽車"    
...
int setDictionary(const char *s1){
  int dic = 7; 
  if (strstr(s1, "unidic")) {
    dic = 8;
  } else if (s1, "ipadic") {
...
SEXP setMeCabMap(int typeSet, char input[], map<string, int> & ma0, 	map<string, int>  &  ma1,  	map<string, int>::iterator & pma0, map<string, int>::iterator & pma,  list <string> & strL, 	 list <string>::iterator & iter,  list <string> & hinsi, 	 list <string>::iterator & hinsi_it,  list <string> & saibun, 	 list <string>::iterator & saibun_it, vector<string> & Ppos2, int pos_n, int Ngram, int genkei, const char * dic ){// map<string, int> &  ma1, char *Ppos[], 
 ...
 int i, j , posC = 0, xx =0; 	
 char buf1 [BUF1];// 2013 12 17 //[128];// [512];//入力された語形を記憶
 char buf2[BUF3];
 char buf3[BUF2];// 2013 12 17 //[64];// [512];記号チェック用
 char buf4[BUF2];// 2013 12 17 //[64];// [1024];記号チェック用
 string str;
 char *p;
  ...
  // int setDic = 7;//
  int setDic = setDictionary(mecab_dictionary_info(mecab)->filename);
  
  // if (strstr(mecab_dictionary_info(mecab)->filename, "unidic")) {
  //               setDic = 9;
  //       } else if (strstr(mecab_dictionary_info(mecab)->filename, "ipadic")) {
  //               setDic = 7;
  //       } else {
  //               printf("Fatal error: the Mecab dictionary file type could not be determined to be either IPADIC or UNIDIC after inspecting the filename (%s)\n", mecab_dictionary_info(mecab)->filename);
  //               return R_NilValue ;
  //       }
  ...
  if(typeSet == 0){// 文字単位なら
	...
	if(strlen(input) > 0){
	  ...
	  for( wz = 0; wz <  wcslen(wbuf) ; wz++){
		...
		// 2008 07 22
		// 2011 04 05 #if defined(_WIN64) || !defined(_WIN32)
	...
	for (;  node; node = node->next) {// node とはその文の形態素ごとに設定される
	  ...
	  if (node->stat == MECAB_BOS_NODE)
	    ...
	    // 2013  		buf1 = (char *)malloc( node->length * MB_CUR_MAX+ 1);	
		strncpy(buf1, node->surface, node->length) ;//元の語形
		
		buf1[node->length] = '\0';// 末尾にNULLを加える// 2009 06 移動
		// strlen関数はstringの文字数を返します。この長さには、終端のNULL文字('\0')は含まれません。
		if(strlen(buf1) < 1){// 2009 06 移動		
		  continue;
		}
		
		//< 2008 11 07> //Rprintf("%s\n", buf1);			
...
//			Rprintf("buf3 %s\n", buf3);
			if(pos_n == 0){
			  ...
			  for(i = 0; i < pos_n; i++){
			    sprintf(buf4, "%s", Ppos2[i].c_str());	// 2014 03 10 sprintf(buf4, "%s", Ppos[i]);				
				//					Rprintf("buf4 %s\n", buf4);
...
// 		// </ 2008 11 08>
		 ...
		 SET_STRING_ELT(my_char2, 0, mkCharCE(p, (utf8locale)?CE_UTF8:CE_NATIVE));
		  ...
		  if(pref == 1){// 原型を取るため，setDic個目-1の要素(0起算で 9 ないし 6番目)を取る
			 z = 0;
			 while(z < (setDic-1)  &&  p!= NULL){
			  p = strtok(NULL, "," );
			  z++;
			}
...
extern "C" {
  SEXP RMeCabText(SEXP filename, SEXP mydic){ 
	...
	mecab = mecab_new2 (dic);// mecab = mecab_new2 ("めかぶ");// mecab_new2 (" -u user.dic");
	CHECK(mecab);
	int setDic = setDictionary(mecab_dictionary_info(mecab)->filename);// 7;
	// if (strstr(mecab_dictionary_info(mecab)->filename, "unidic")) {
        //         setDic = 9;
        // } else if (strstr(mecab_dictionary_info(mecab)->filename, "ipadic")) {
        //         setDic = 7;
...
// 	}
	...
	if((fp = fopen(file, "r")) == NULL){
	  ...
	  while(!feof(fp)){
	    if(fgets(input2, FILEINPUT, fp) != NULL){// 2014 03 11 if(fgets(input2, 5120, fp) != NULL){
...
extern "C" {
 ...
 int setDic =  setDictionary(mecab_dictionary_info(mecab)->filename);// 7;7;
...
// // </ 2008 11 08>
	  ...
	  SET_STRING_ELT(VECTOR_ELT(mydf, 1), z, mkCharCE(token, (utf8locale)?CE_UTF8:CE_NATIVE));///////<2009 04 18>
...
// // </ 2008 11 08>
	  ...
	  SET_STRING_ELT(VECTOR_ELT(mydf, 2), z, mkCharCE(token, (utf8locale)?CE_UTF8:CE_NATIVE));	 ///////<2009 04 18>
...
//  // </ 2008 11 08>
	  ...
	  SET_STRING_ELT(VECTOR_ELT(mydf,0), z, mkCharCE(token, (utf8locale)?CE_UTF8:CE_NATIVE)); ///////<2009 04 18>
	...
	pc++;
	char labelbuff[6];//char labelbuff[n]; 2009 03
	for ( z = 0; z < n; z++) {
	    ...
	    if(fgets(input2, FILEINPUT, fp) != NULL){// 2014 03 11 if(fgets(input2, 5120, fp) != NULL){
		  ...
		  for (;  node; node = node->next) {
			...
			if (node->stat == MECAB_BOS_NODE)
			  ...
			  buf1[node->length] = '\0';// NULL-Terminateしておく// 2009 06 移動
...
extern "C" {
  ...
  if(pos_n > 0 && flag){
    PROTECT(myPos = AS_CHARACTER(pos));pc++;
	  // 2014 03 10 for( i = 0; i < pos_n; i++){
	  // Ppos[i] = R_alloc( (unsigned int ) strlen (CHAR(STRING_ELT(myPos, i))), sizeof(char));
	  // }
  //   Rprintf("end myPos = AS_CHARACTER(pos) \n");
    for( i = 0; i < pos_n; i++){
	  // 2014 03 10 strcpy(Ppos[i], CHAR(STRING_ELT(myPos, i)));
      Ppos2.push_back(CHAR(STRING_ELT(myPos, i)));  // 2014 03 10 
	//	Rprintf("Pos[%d] =  %s\n", i, Ppos[i]);
...
// 	  strcpy(Ppos[0], meisiCode());
	...
	// 2014 03 10 // Ppos[0] = R_alloc(  (unsigned int )  strlen( meisiCode()), sizeof(char));
	// 2014 03 10 // strcpy(Ppos[0], meisiCode() );
	Ppos2.push_back( meisiCode() ); // 2014 03 10 
	// 2014 03 10 //  Ppos[1] = R_alloc( (unsigned int )  strlen( keiyouCode() ), sizeof(char));
	// 2014 03 10 //  strcpy(Ppos[1], keiyouCode() );
	Ppos2.push_back( keiyouCode() ); // 2014 03 10 
...
// //	int test = 0;
	...
	int pos_n  = INTEGER_VALUE( posN );// pos の数 // 2008 06 13
	...
	if(pos_n > 0 && flag){
	  ...
	  pc++;
	  // 2014 03 10 // for( i = 0; i < pos_n; i++){
		// 2014 03 10 // Ppos[i] = R_alloc(strlen(CHAR(STRING_ELT(myPos, i))), sizeof(char));
	  // 2014 03 10 // }
	  //   Rprintf("end myPos = AS_CHARACTER(pos) \n");
	  for( i = 0; i < pos_n; i++){
		// 2014 03 10 //  strcpy(Ppos[i], CHAR(STRING_ELT(myPos, i)));
...
// 	char gram[6*3 + 1] = {0};
	...
	char buf1[BUF1];// [512];//入力された語形を記憶 // 2012 04 03
	char buf2[BUF3];// 2012 04 03
	int totalN = 0;// 2012 04 03
...
// 	}
	...
	if((fp = fopen(file, "r")) == NULL){
	  ...
	  while(!feof(fp)){
	    if(fgets(input, FILEINPUT, fp) != NULL){// 2014 03 11 if(fgets(input, 5120, fp) != NULL){		  
...
// 			  //
// 			  //for(int z = 0; z <  (wcslen(wbuf) - Ngram); z++){
			  for( wz = 0; wz <  wcslen(wbuf) ; wz++){	// 2008 07 22
...
// 				}//if_end
				...
				while ( p != NULL ) {
				 if(typeSet == 2){// 品詞情報で数える
				   ...
				   if( j == setDic){// j == 7 // p には形態素原型がセットされている状態
					 //////// < 2008 06 13>
...
// // </ 2008 11 08>
		  ...
		  SET_STRING_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE( (pa->first).c_str(), (utf8locale)?CE_UTF8:CE_NATIVE ));			  ///<2009 04 18>
		  
		  INTEGER(VECTOR_ELT(mydf,1))[z] =   pa->second;// 最後に頻度情報
		  pa++;
		  // if(z % 10 == 0)	  Rprintf("* ");// 2009 03 removed 2010 05
		}
...
// 		// </ 2008 11 08>
	...
	///////////////////////// new_end 
	  ...
	  // 行名を指定．必須
		...
		pc++;
		char labelbuff[6];// char labelbuff[nn]; 2009 03
		for ( z = 0; z < nn  ; z++) {
		  sprintf(labelbuff, "%d", z+1);
...
extern "C"{
  ...
  SEXP collocate(SEXP filename, SEXP target, SEXP span, SEXP mydic ){
	...
	mecab = mecab_new2 (dic);// mecab = mecab_new2 ("めかぶ");// mecab_new2 (" -u user.dic");// 2012 04 03 
	CHECK(mecab);// 2012 04 03 
...
// 			}
	...
	//  その単純な初期化
	  ...
	  for( z = 0; pM != mP.end(); z++, pM++){
		...
		SET_STRING_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE( (pM->first).c_str(), (utf8locale)?CE_UTF8:CE_NATIVE ));	//<2009 04 18> ターム名の登録
	  ...
	  // 	SET_STRING_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE( (pa2->first).c_str(), (utf8locale)?CE_UTF8:CE_NATIVE ));	//<2009 04 18>
...
//  // </ 2008 11 08>
	  ...
	  SET_STRING_ELT(VECTOR_ELT(mydf, 0), n-1, mkCharCE( "[[TOKENS]]", (utf8locale)?CE_UTF8:CE_NATIVE)); //<2009 04 18>
...
extern "C" {
  SEXP NgramDF(SEXP filename, SEXP type, SEXP N, SEXP pos, SEXP posN , SEXP mydic  ){
	...
	int Ngram  = INTEGER_VALUE( N );// 最小頻度の数
	int mFreq = 2;
	char  input[10240];// 2013 12 17 ;// [5120] = { 0 };//char  input[5120] = { 0 };
	//	char  input2[5120] = { 0 };
	
	wchar_t  wbuf [10240];// 2013 12 17 ;// [5120] = { 0 }; //wchar_t  wbuf [5120] = { 0 }; /* ワイド文字列 : 日本語文字数  + 1 */
	FILE *fp;
	char * p;
	int i, j, n, nn, z, xx ;
	unsigned int  wz = 0;
	...
	//////////////////////// < 2008 06 13 >
	char buf3[BUF3];// 2013 12 17 ;// [64]; // [1024];品詞チェック用
	char buf4[BUF3];// 2013 12 17 ;[64]; // [1024];品詞チェック用
	int pos_n  = INTEGER_VALUE( posN );// pos の数 // 2008 06 13
	...
	SEXP mydf, tmp, varlabels, row_names;// 2012 04 03
	...
	char buf1[BUF3];// 2013 12 17 ;[128];// [512];//入力された語形を記憶
	char buf2[BUF3];
...
// 	  strcpy(Ppos[0], meisiCode());
	  ...
	  // 2014 03 10 // Ppos[0] = R_alloc(strlen( meisiCode()), sizeof(char));
	  // 2014 03 10 // strcpy(Ppos[0], meisiCode() );
	  Ppos2.push_back ( meisiCode() ) ;// 2014 03 10 
	  // 2014 03 10 // Ppos[1] = R_alloc(strlen( keiyouCode() ), sizeof(char));
	  // 2014 03 10 // strcpy(Ppos[1], keiyouCode() );
	  Ppos2.push_back ( keiyouCode() ) ;// 2014 03 10 
	}
	//////////////////////// </ 2008 06 13>
		...
		/////////////// new_begin ////////////////
				  ...
				  if(strL.size() >= (unsigned int) Ngram){
					...
					for ( iter = strL.begin(); iter != strL.end(); iter++){
					  ...
					  target.append( *iter);
					  if(xx < Ngram){
						target.append(" ");	// target.append("-");// 2008 06 14 
					  }
					  xx++;
					  //					  Rprintf("xx = %d\n", xx);
...
//   // </ 2008 11 08>
			  ...
			  SET_STRING_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE( buf3 , (utf8locale)?CE_UTF8:CE_NATIVE));	///<2009 04 18>		 
...
//   // </ 2008 11 08>
		...
		SET_VECTOR_ELT(VECTOR_ELT(mydf, 0), z, mkCharCE( " " ,  (utf8locale)?CE_UTF8:CE_NATIVE));///<2009 04 18>		 
...
//   // </ 2008 11 08>
			  ...
			  SET_STRING_ELT(VECTOR_ELT(mydf, j++), z, mkCharCE( buf3, (utf8locale)?CE_UTF8:CE_NATIVE ));	///<2009 04 18>
...
extern "C" {
  ...
  SEXP docMatrixDF(SEXP charVec, SEXP charLeng, SEXP pos, SEXP posN, SEXP minFreq, SEXP mydic  ){ 
	...
	char *p;
	SEXP ans, dim, dimnames, row_names, col_names;// 2012 04 03 
	int mFreq  = INTEGER_VALUE( minFreq );// 最小頻度の数
	PROTECT(charLeng = AS_INTEGER(charLeng));pc++;
	int file_num = INTEGER_POINTER(charLeng)[0];
	PROTECT(charVec = AS_CHARACTER(charVec));pc++;
		  	
	map<string, int> ma0;//, ma[file_num];     // 全要素用のマップと要素ごとのマップ // 2012 04 03
	vector<map<string, int> > vecmap; // 2014 03 09 
	for (i = 0; i < file_num; i++) vecmap.push_back(map<string, int>() );
	
	map<string, int>::iterator pma0, pma;// マップ検索用// 2012 04 03
	...
	int pos_n  = INTEGER_VALUE( posN );// pos の数 // 2008 06 3
	bool flag = 1;
	if(pos_n < 1 ){
	  pos_n = 2;// = 1
	  flag = 0;
...
// 		  }
		  ...
		  while ( p != NULL ) {
			...
			if(j == 1){//品詞情報1
			  ...
			  //Rprintf("in str = p\n");			  
			  for(z = 0; z < pos_n; z++){
			    sprintf(buf4, "%s", Ppos2[z].c_str());// 2014 03 10 // sprintf(buf4, "%s", Ppos[z]);
				sprintf(buf3, "%s", p);
				//Rprintf("buf3 %s\n", buf3);
				if(strcmp(buf3, buf4) == 0){
				  posC = 1;
...
// 	  }
	...
	while( pma0 != ma0.end()  ){
	  if(pma0->second < mFreq){
		...
		ma0.erase(pma0++);///// ma0.erase(pma0);// 2010 09 15 // ma0.erase(pma0++);
	  }else{
		++pma0;
	  }
	}
...
//  // </ 2008 11 08>
	  ...
	  pma0++;
	  // if(i % 10 == 0) Rprintf("* ");// 2009 03 27 removed 2010 05
	}
	Rprintf("\n");// 2009 03 27
  // 行名のセット// これは要素番号
...
extern "C" {
  ...
  SEXP docMatrix2(SEXP directory, SEXP origF, SEXP fileN, SEXP ft, SEXP pos, SEXP posN, SEXP minFreq, SEXP kigo , SEXP mydic  ){// SEXP sym, SEXP kigo 
	...
	FILE *fp; // 2012 04 03
	map<string, int> ma0;// , ma[f_count];     // ファイル数の数+登録単語用マップの数1 // 2012 04 03
	vector <map<string, int> > vecmap;// 2014 03 09 
	...
	// 2014 03 10 // char * f[f_count];
	vector <string> ff;
	PROTECT(origF = AS_CHARACTER(origF));pc++;
	// 2014 03 10 //  for(file = 0; file < f_count; file ++){
 		// 2014 03 10 // f[file]  =  R_alloc(strlen(CHAR(STRING_ELT(origF, file))), sizeof(char));
	// 2014 03 10 // }
	for(file = 0; file < f_count; file ++){
  		// 2014 03 10 //  strcpy(f[file], CHAR(STRING_ELT(origF, file)));
		ff.push_back( CHAR(STRING_ELT(origF, file))) ; // 2014 03 10 
	}
...
// 	  Ppos[0] = R_alloc(strlen(CHAR(STRING_ELT(myPos, 0))), sizeof(char));
  ...
  //       }
	...
	for(file = 0; file < f_count; file++) {
	  ...
	  sprintf(file_name, "%s/%s", path, ff[file].c_str());// 2014 03 10  // sprintf(file_name, "%s/%s", path, f[file]);
...
//   // </ 2008 11 08>
  ...
  // 行名のセット// これはファイル名
	...
	for(file = 0; file < f_count;file++){ 
	  //	  Rprintf("row names = %s\n", f[file]);
	  strcpy(buf3, ff[file].c_str() );// 2014 03 10 // strcpy(buf3, f[file]);// strcpy(buf3, *pv);
	  //	  if(mFt == 1){
...
// 				  }
				...
				if(strL.size() >= (unsigned int) Ngram){// リストのサイズが指定通りであるなら，保存を始める
				  ...
				  pma = (vecmap.at(file)).find(target);// ma[file].find(target);// str 2009 08 25 出てきた形態素原型は既に個別マップにあるか？
				  if(pma !=  (vecmap.at(file)).end()){// if(pma != ma[file].end()){
					pma->second =  pma->second + 1;
					//二つ目の数値を加算
				  }
...
extern "C" {
  ...
  SEXP NgramDF2(SEXP directory, SEXP origF, SEXP fileN, SEXP ft, SEXP type, SEXP pos, SEXP posN, SEXP minFreq, SEXP N, SEXP kigo , SEXP mydic ){ // SEXP sym ,SEXP kigo 
	...
	PROTECT(directory = AS_CHARACTER(directory));pc++;// 2012 04 03
	path = R_alloc(strlen(CHAR(STRING_ELT(directory, 0))), sizeof(char));
	strcpy(path, CHAR(STRING_ELT(directory, 0)));
...
// 		continue;
	  ...
	  if((fp = fopen(file_name, "r")) == NULL){
		...
		while(!feof(fp)){
		  ...
		  if(fgets(input, FILEINPUT, fp) != NULL){// 2013 03 11  if(fgets(input, 5120, fp) != NULL){
			if(strlen(input) < 1){
			  continue;
			}
...
//   // </ 2008 11 08>
		...
		SET_STRING_ELT(VECTOR_ELT(mydf, j++), xx, mkCharCE( buf3 , (utf8locale)?CE_UTF8:CE_NATIVE));// < 2009 04 18>
...
// 	while(pma0 != ma0.end()){
	for(file = 0; file < f_count; file ++){
	  ...
	  // Rprintf("* ");// 2009 03 27 removed 2010 05
	}
...
//   // </ 2008 11 08>
	  ...
	  SET_STRING_ELT(varlabels, xx, mkCharCE(ff[j++].c_str() , (utf8locale)?CE_UTF8:CE_NATIVE));// 2014 03 10 SET_STRING_ELT(varlabels, xx, mkCharCE(f[j++], (utf8locale)?CE_UTF8:CE_NATIVE));
	...
	char labelbuff[6];// char labelbuff[2]; 2009 03
	for (xx = 0; xx < n0 ; xx++) {
...
// 			 SEXP kigo,
	...
	char* path = 0;// 2014 03 11  char* path;
	// 2014 03 10 //  char * f[f_count];
	vector <string> ff;
	//const char* KIGO = CHAR(STRING_ELT(kigo,0));
	...
	SEXP tmp, row_names, mydf = R_NilValue, varlabels = R_NilValue;// 2014 03 11 //  SEXP mydf, tmp, row_names, varlabels;// SEXP ans, dim, dimnames, row_names, col_names;
	...
	list <string>::iterator hinsi_it, iter, saibun_it;// 2012 04 03
	...
	if(mFt == 1 || mFt == 0 ){// ファイル 0 かディレクトリ 1
	  ...
	  for(file = 0; file < f_count; file++){
		...
		// 2014 03 10 // Rprintf("f[file] = %s\n", f[file]); // 2014 03 09
		// 2014 03 10 // Rprintf("ff[file] = %s\n", ff[file].c_str()); // 2014 03 09
	  }
	}
...
// #endif
			...
			SET_STRING_ELT(VECTOR_ELT(mydf, 0), xx, mkCharCE( buf3, (utf8locale)?CE_UTF8:CE_NATIVE ));// < 2009 04 18>
...
// #endif
				...
				SET_STRING_ELT(VECTOR_ELT(mydf, j++), xx, mkCharCE( buf3, (utf8locale)?CE_UTF8:CE_NATIVE  ));//j列のxx行にセット < 2009 04 18>
...
// 			  }
		...
		// 各ファイルから探し出してその頻度を新規列に追加
			...
			pma0++;
			// if(xx % 10 == 0) Rprintf("* ");// 2009 03 27 removed 2010 05
...
// #endif
		  ...
		  SET_STRING_ELT(varlabels, j+1, mkCharCE(ff[j].c_str(), (utf8locale)?CE_UTF8:CE_NATIVE )); // 2014 03 10 SET_STRING_ELT(varlabels, j+1, mkCharCE(f[j], (utf8locale)?CE_UTF8:CE_NATIVE ));
...
// #endif
	  ...
	  if(mFt == 0 || mFt == 1){//  各ファイル名を列名として設定
		for(j = 0; j < f_count; j++){
		  ...
		  sprintf(buf4, "%s", ff[j].c_str());// 2014 03 10 sprintf(buf4, "%s", f[j]);//s
...
// #endif
	...
	char  labelbuff[6];// char  labelbuff[5]; 2009 03
	for (xx = 0; xx < n0 ; xx++) {
	  sprintf(labelbuff, "%d", xx+1);
...
//        }
	...
	map<string, int> ma0;// , ma[row_num];     // ファイル数の数+登録単語用マップの数1 // 2012 04 03
	vector <map<string, int>  > vecmap;// 2014 03 09 
	...
	if(pos_n > 0 && flag){
	  ...
	  for( i = 0; i < pos_n; i++){
		...
		Ppos2.push_back ( CHAR(STRING_ELT(myPos, i))); // 2014 03 10 
		//	Rprintf("Pos[%d] =  %s\n", i, Ppos[i]);
		if(strcmp(Ppos2[i].c_str(), KIGO) == 0){// if(strcmp(Ppos[i], KIGO) == 0){// 2014 03 10 
		  mSym = 1;
		}
	  }
	}// 2008 06 23
...
// 			}
	  ...
	  // if(file % 10 == 0) Rprintf("* ");// 2009 03 27 removed 2010 05
